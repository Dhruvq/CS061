...
STR R7, R6, #0
    ...
    GETC
    ADD R2, R0, #-10 ;compare input to newline
    BRz END_INPUT_LOOP ;input is newline
    
    STR R0, R1, #0 ;store to R1 address
    OUT
    ADD R1, R1, #1 ;increment R1 address
    ADD R5, R5, #1 ;incremment char counter
    
    BR INPUT_LOOP
    END_INPUT_LOOP
    
    LD R0, NULL_CHAR ;ascii 0 > R0
    STR R0, R1, #0 ;append 0 to array
...
.orig x3600
    ...
    AND R3, R3, R2  ;Bitmask of decimal 32(xFDF) which is 100000
    STR R3, R1, #0  
    ADD R1, R1, #1 
    BR CHANGE_CASE  ;repeat
